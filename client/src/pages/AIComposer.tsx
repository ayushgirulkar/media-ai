...
const AIComposer = () => {
  ...
  const [generation, setGenerations] = useState<any[]>([]);
  ...
  const tones = ["Profetional", "Creative", "Funny", "Minimalist", "Excited"];
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        <h1 className="text-3xl text-slate-700 tracking-tight">
          What you whant to create ?
        </h1>

        <div className="relative group mt-12">
          <textarea
            className="w-full px-6 py-6 bg-white border border-slate-300 rounded-xl
            text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-40"
            placeholder="whats your idea ?..(ex. A post about the new launch of product)"
            value={prompt}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            ...
            >
              <div className="flex flex-col h-full space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className="text-xs test-red-500 bg-red-50 px-2 py-0.5
                    round-md"
                  >
                    {gen.tone}
                  </span>
                ...
                {gen.mediaUrl && (
                  <div
                    className="rounded-xl overflow-hidden border border-slate-50
                    bg-slate-50"
                  >
                    <img
                      src={gen.mediaUrl}
                ...
                <div className="flex items-center gap-2 pt-2">
                  <button
                    ...
                    className="flex-1 bg-slate-100 hover:bg-red-500
                    hover:text-white text-slate-600 text-xs py-2.5 rounded-lg transition-all
                    "
                  >
                    Shedule Post
                  </button>
          ...
          {generation.length === 0 && (
            <div className="col-span-full py-20 text-center space-y-2">
              <div
                className="size-12 bg-slate-50 rounded-2xl flex items-center
      justify-center mx-auto text-slate-300"
              ...
              <p className="text-slate-500">
                No content generated till now. Try to generate content using AI.
              </p>
            </div>
          )}
        </div>
      ...
      {activeScheduler && (
        <div
          className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 
        bg-slate-900/40 backdrop-blur-md animated-in fade-in duration-300"
        >
          <div
          ...
          >
            ...
            >
              <h3 className="text-slate-900">Scheduel Generation</h3>
              <button
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="relative">
                    <Clock2Icon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                    <input
                      type="time"
                      className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-100 rounded-md text-slate-900 text-sm focus:outline-none transition-all"
                      value={scheduleTime}
              ...
              <button onClick={handleSchedule} className="w-full flex items-center
              justify-center gap-2 py-3 rounded-md bg-slate-200 text-slate-700
              hover:bg-red-500 hover:text-white transition">
                {scheduling?<LoaderCircleIcon className="size-4 animate-spin"/>:<TimerResetIcon className="size-4"/>}
                Schedule Post
              </button>
            </div>
          </div>
