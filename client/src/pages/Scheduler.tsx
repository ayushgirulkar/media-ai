...
import { ArrowRight, ArrowRightIcon, Calendar, CalendarDaysIcon, CalendarIcon, Clock10Icon, SendHorizontalIcon, XIcon } from "lucide-react";
            ...
            {/* Media Upload */}
                ...
                {mediaFile?(
                  <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                    ...
                    hover:bg-slate-900/80 text-white rounded-full flex items-center justify-center transition-colors">
                  ...
                  hover:border-red-300 hover:bg-red-50/30 transition-all group">
                    <span className="text-sm test-slate-500 group-hover:text-red-600 transition-colors">Click to upload image or video</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <div className="relative">
                  ...
                  bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-null" value={scheduledDate}onChange={(e)=>setScheduledDate(e.
                    target.value)} />
              ...
              <div>
                <label className="block text-xs text-slate-500 uppercase mb-2">Date</label>
                <div className="relative">
                  <Clock10Icon className="size-4 absolute left-3 top-1/2 -translate-y-1/2
                  text-slate-400 pointer-events-none"/>

                  <input type="time" required className="w-full pl-10 pr-4 py-2.5
                  bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm outline-null" value={scheduledTime}onChange={(e)=>setScheduledTime(e.
                    target.value)} />
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
        {/*upcomming posts*/}
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="flex iitems-center gap-2.5 px-5 py-4 border-b border-slate-100">
                  <CalendarDaysIcon className="size-4 text-zinc-500"/>
                  <h3 className="text-slate-900 text-sm">Upcoming Posts</h3>
                  <span className="ml-auto text-xs font-bold bg-zinc-100 text-zinc-700 px-2
                  py-0.5 rounded-full">{scheduled.length}</span>
                </div>
                <div className="max-h-72 overflow-y-auto divided-y divide-slate-50">
                  {scheduled.length===0?(
                    ...
                    scheduled.map((post)=>(
                      ...
                      transition-colors">
                        ...
                        <p className="text-sm text-slate-500 line-clamp-2 max-w-md">{post.content}
                              Content
                        </p>
         ...
         <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="flex iitems-center gap-2.5 px-5 py-4 border-b border-slate-100">
                  <SendHorizontalIcon className="size-4 text-zinc-500"/>
                  <h3 className="text-slate-900 text-sm">UPublished Posts</h3>
                  <span className="ml-auto text-xs font-bold bg-zinc-100 text-zinc-700 px-2
                  ...
                  {published.length===0?(
                    ...
                    published.map((post)=>(
                      ...
                      transition-colors">
                        ...
                        <p className="text-sm text-slate-500 line-clamp-2 max-w-4/5">{post.content}
                              Content
                        </p>
