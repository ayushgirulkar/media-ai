...
import {UsersIcon,CalendarDaysIcon,Wand2Icon, LayoutDashboardIcon, LogOutIcon,} from 'lucide-react'
...
const Sidebar = ({isOpen, setIsOpen}: {isOpen: boolean, setIsOpen: (val: boolean) => void}) => {
    ...
    const {logout,user}={
        ...
        user:{name:"John Doe",email:"jhon@gmail.com"} 
...
const navItems = [
  ...
  return (
    ...
    flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {navItems.map((item)=>{
          ...
          return (
            <NavLink
              ...
              className={`flex items-center gap-3 px-3 py-2.5 text-sm transition-all duration-150 border ${
                isActive
      ...
      <div className="p-4 border-t border-slate-100">
        <div className="flex items-center gap-3 p-2 rounded-x1 hover:bg-slate-50 transition-colors">
